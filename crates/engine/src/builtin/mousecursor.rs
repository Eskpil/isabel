...
use crate::{PluginMessage, ShellCapabilities};
...
pub struct Mousecursor {
    tx: Option<Sender<crate::PluginMessage>>,
}
...
impl crate::Plugin for Mousecursor {
    fn init(
        &mut self,
        tx: Sender<crate::PluginMessage>,
        shell_capabilities: crate::ShellCapabilities,
    ) -> anyhow::Result<()> {
        self.tx = Some(tx);
    ...
    fn handle(&mut self, payload: Vec<u8>) -> anyhow::Result<()> {
        let message: MouseCursor = codec::from_slice(&payload[..])?;

        match message {
            MouseCursor::ActivateSystemCursor { device, kind } => {
                let icon = match kind {
