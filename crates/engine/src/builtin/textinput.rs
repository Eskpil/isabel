...
use textmodel::{TextModel, TextRange};
use xkeysym::Keysym;
...
impl Textinput {
    ...
    fn handle_enter_pressed(&mut self) {
        ...
        publish(self.on().to_owned(), &method, self.tx.clone().unwrap());
    }
    ...
    pub fn publish_state(&mut self) {
        ...
        publish(self.on().to_owned(), &cmd, self.tx.clone().unwrap());
    }
}

impl crate::Plugin for Textinput {
    fn init(
        &mut self,
        tx: calloop::channel::Sender<crate::PluginMessage>,
        _: crate::ShellCapabilities,
    ) -> anyhow::Result<()> {
        self.tx = Some(tx);
        Ok(())
    }
