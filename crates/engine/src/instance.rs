...
use calloop::{channel, timer::TimeoutAction};
...
impl Instance {
    pub fn new(backend: Backend) -> anyhow::Result<Self> {
        ...
        let userdata = Userdata {
            backend,
            plugins: vec![],
            task_runner: tasks::TaskRunner::new()?,
        };
        ...
        task_runner_description.post_task_callback = Some(tasks::TaskRunner::post_task);
        task_runner_description.runs_task_on_current_thread_callback =
            Some(tasks::TaskRunner::runs_on_thread);
        ...
        let assets_path = CString::new("demo/build/flutter_assets")?;
        let icu_data_path = CString::new("demo/icudtl.dat")?;
    ...
    pub fn run<'a, T: Shell>(
            ...
            .insert_source(stream, move |e, _, s| {
                if let calloop::channel::Event::Msg(event) = e {
                    match event {
                        ...
                        stream::Event::PointerMotion { x, y, time, serial }=> {
                            unsafe {
                                ...
                                if event.phase == bindings::FlutterPointerPhase_kMove {
                                    let plugin = s.instance_mut().userdata.plugins.iter_mut().find(|p| p.on() == "isabel/decorations").unwrap();
                                    let decorations = plugin.downcast_mut::<Decorations>().unwrap();
                        ...
                        stream::Event::PointerButton { x, y, button, time, pressed, serial } => {
                            unsafe {
                                let mut event = std::mem::zeroed::<bindings::FlutterPointerEvent>();
                                event.struct_size = std::mem::size_of::<bindings::FlutterPointerEvent>();
        ...
        handle
            .insert_source(
                calloop::timer::Timer::from_duration(Duration::from_nanos(200)),
                move |_, _, s| {
